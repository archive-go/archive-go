...
// 获取单独的知乎回答，爬虫解决静态页面
func getSingleAnswer(url string, page *telegraph.Page) error {
...
// 获取单独的知乎专栏，爬虫解决静态页面
func getSingleZhuanLan(url string, page *telegraph.Page) error {
	spider := colly.NewCollector()
	extensions.RandomUserAgent(spider)
	extensions.Referer(spider)
